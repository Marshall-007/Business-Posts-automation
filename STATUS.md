...
| Per-campaign captions: your own captions rotated across posts (or auto) | BUILT | engine + Auto Posts controls + tests |
...
| Test suite | GREEN | 86 tests, CI on every push |
...
- **Per-campaign captions & hashtags** (Auto Posts -> each month ->
  "Captions & hashtags"):
  - *Captions:* choose **auto** (a unique caption per post) or **my own
    captions**. In the latter, add/edit/remove a list of captions; the engine
    rotates one onto each post in order and repeats the list once it runs out
    (add just one caption to use the same caption on every post). Sidecar and
    folder caption.txt files still take precedence when present.
  ...
  - The chosen caption is frozen into the queue, so `preview.html` shows exactly
    what will go out.
